...
#  ## Kodeliste for opptaksområder i spesialisthelsetjenesten (PHV) t-1

# +
opptaksomrader_KLASS <- klassR::GetKlass(630, output_style = "wide", date = c(paste0(aargang-1, "-01-01"))) %>%
...
opptaksomrader_PHV_HF_filsti <- paste0("ssb-prod-helse-speshelse-data-kilde/felles/Kart/", aargang-1, "/Opptaksområder/opptaksomrader_PHV_HF_flate", "_", aargang-1, "/")

opptaksomrader_PHV_HF <- open_dataset(opptaksomrader_PHV_HF_filsti) %>%
...
test_t1 <- sf::st_intersection(grunnkrets_kart_t1, opptaksomrader_PHV_HF) %>%
data.frame() %>%
dplyr::select(GRUNNKRETSNUMMER, NAVN_HF) %>%
dplyr::rename(NAVN_HF_T1 = NAVN_HF)

test <- sf::st_intersection(grunnkrets_kart, opptaksomrader_PHV_HF) %>%
data.frame() %>%
dplyr::select(GRUNNKRETSNUMMER, NAVN_HF) %>%
dplyr::rename(NAVN_HF_NY = NAVN_HF)
...
endringer_2 %>%
dplyr::filter(NAVN_HF_T1 != NAVN_HF_NY)
# -
...
# +
slett <- endringer_2 %>%
dplyr::distinct(GRUNNKRETSNUMMER_T1, sourceName, NAVN_HF_T1)
...
dplyr::distinct(GRUNNKRETSNUMMER, targetName, NAVN_HF_NY)
