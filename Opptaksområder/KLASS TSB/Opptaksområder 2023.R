...
#  ## Kodeliste for opptaksområder i spesialisthelsetjenesten (TSB) t-1
...
opptaksomrader_TSB_HF_filsti <- paste0("ssb-prod-helse-speshelse-data-kilde/felles/Kart/", aargang-1, "/Opptaksområder/opptaksomrader_TSB_HF_flate", "_", aargang-1, "/")

opptaksomrader_TSB_HF <- open_dataset(opptaksomrader_TSB_HF_filsti) %>%
...
test_t1 <- sf::st_intersection(grunnkrets_kart_t1, opptaksomrader_TSB_HF) %>%
data.frame() %>%
dplyr::select(GRUNNKRETSNUMMER, NAVN_HF) %>%
dplyr::rename(NAVN_HF_T1 = NAVN_HF)

test <- sf::st_intersection(grunnkrets_kart, opptaksomrader_TSB_HF) %>%
data.frame() %>%
dplyr::select(GRUNNKRETSNUMMER, NAVN_HF) %>%
