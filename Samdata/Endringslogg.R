...
laste_skjema <- function(skjema) {
  infile <- paste0("Skj", tolower(skjema), "_")
  return(infile)
}
...
if(identical(nyeste_versjon, character(0)) == TRUE) {
  print(paste0("Script stoppet: Nyeste versjon av skjema", skjema, "finnes ikke"))
  stop()
}

if(identical(nest_nyeste_versjon, character(0)) == TRUE) {
  print(paste0("Script stoppet: Nest nyeste versjon av skjema", skjema, "finnes ikke"))
  stop()
}
# -
...
ny_fil <- read.csv(textConnection(gsub("¤", "|", readLines(nyeste_versjon, encoding = "latin1"))),
                   sep = "|",
                   na.strings=c(".", "", " "),
                   fill = TRUE,
                   header = TRUE,
...
gammel_fil <- read.csv(textConnection(gsub("¤", "|", readLines(nest_nyeste_versjon, encoding = "latin1"))),
                       sep = "|",
                       na.strings=c(".", "", " "),
                       fill = TRUE,
                       header = TRUE,
...
# OBS: fix for gamle filer som ble sendt som UTF-8. Slett når alle gamle filer også er latin1
if ("HELSEREGIONÂ" %in% colnames(gammel_fil)) {
  gammel_fil <- read.csv(textConnection(gsub("¤", "|", readLines(nest_nyeste_versjon, encoding = "UTF-8"))),
                         sep = "|",
                         na.strings=c(".", "", " "),
                         fill = TRUE,
                         header = TRUE,
...
if (file.exists(paste0(endringslogg_mappe, "/", filnavn_eksport(skjema), sep = ""))==FALSE){

  write.xlsx(summary, file = paste0(endringslogg_mappe, "/", filnavn_eksport(skjema), sep = ""), sheetName = "Antall enheter og variabler", rowNames = FALSE, showNA=FALSE)
  ...
  print(paste0("Filen ", filnavn_eksport(skjema), "finnes allerede"))
  stop()
...
if (nrow(variabler_mangler) > 0) {
  ...
  print("(Hvilke variabler finnes i en, men ikke i begge filene): Ingen variabler mangler")
}
...
if (skjema %in% skjemaliste) {
  ...
  enheter_mangler <- dplyr::left_join(enheter_mangler, foretak_info, by = c("FORETAKETS_ORGNR")) %>%
    select(FORETAKETS_ORGNR, FORETAKETS_NAVN, HELSEREGION_NAVN, 'Finnes i fil', 'Mangler i fil')
  ...
  enheter_mangler <- dplyr::left_join(enheter_mangler, finst_info, by = c("FINST_ORGNR")) %>%
    select(FINST_ORGNR, FINST_NAVN, HELSEREGION_NAVN, 'Finnes i fil', 'Mangler i fil')
    }
...
if (nrow(enheter_mangler) > 0) {
  ...
  print("Hvilke enheter (FINST_ORGNR/FORETAKETS_ORGNR) finnes ikke i begge filene?: Ingen enheter mangler")
}
# -
...
if (nrow(forskjeller_per_variabel) > 0) {
  ...
  print("(Antall forskjellige observasjoner per variabel): Ingen forskjeller mellom noen av variablene")
}
# -
...
if (nrow(ulike_observasjoner) > 0) {
  ...
  if (skjema %in% skjemaliste) {
    ...
    ulike_observasjoner <- list(ulike_observasjoner, antall_ulike_observasjoner_per_enhet, foretak_info) %>%
      purrr::reduce(left_join, by = "FORETAKETS_ORGNR") %>%
      select(FORETAKETS_ORGNR, FORETAKETS_NAVN, HELSEREGION_NAVN, Variabel, 'Verdi nyeste versjon', 'Verdi nest nyeste versjon', 'Antall variabler med forskjellige verdier (per enhet)')
    ...
    ulike_observasjoner <- list(ulike_observasjoner, antall_ulike_observasjoner_per_enhet, finst_info) %>%
      purrr::reduce(left_join, by = "FINST_ORGNR") %>%
      select(FINST_ORGNR, FINST_NAVN, HELSEREGION_NAVN, Variabel, 'Verdi nyeste versjon', 'Verdi nest nyeste versjon', 'Antall variabler med forskjellige verdier (per enhet)')
...
if (nrow(ulike_observasjoner) > 0) {
  ...
  # showDialog("Hvilke observasjoner for (FINST_ORGNR/FORETAKETS_ORGNR) som er ulike i begge filene", "Alle variablene har identiske verdier i begge filene")
    print("Alle variablene har identiske verdier i begge filene")
}

# Lagrer filen #
openxlsx::saveWorkbook(wb, paste0(endringslogg_mappe, "/", filnavn_eksport(skjema)), overwrite = TRUE)
# -
